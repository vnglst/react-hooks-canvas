...
function usePersistantState(initial) {
  const [locations, setLocations] = useState(
    JSON.parse(localStorage.getItem("my-app")) || initial
  );

  useEffect(() => {
    localStorage.setItem("my-app", JSON.stringify(locations));
  });

  return [locations, setLocations];
}

function useCanvas(cb, props) {
...
function App() {
  const [location, setLocations] = usePersistantState([]);

  const canvasRef = useCanvas(canvas => {
    const ctx = canvas.getContext("2d");
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    location.forEach(point => draw(ctx, point));
  });
  ...
  function handleDraw(event) {
    ...
    setLocations([...location, newPoint]);
  }
